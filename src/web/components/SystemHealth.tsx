  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs text-zinc-400">
        <span>{label}</span>
        <span className="font-mono">{used} / {total} MB <span className="text-zinc-500">({pct}%)</span></span>
      </div>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3 text-zinc-500">
        ...
        <p className="text-xs text-zinc-600">This may take a few seconds while SSH connections are verified</p>
      </div>
    );
  }

  const onlineCount = data?.environments.filter((e) => e.status === "online").length ?? 0;
      ...
      {data && (
        ...
        >
          ...
          <div className="flex-1">
            <p className={cn("font-bold text-sm", allOnline ? "text-emerald-400" : "text-amber-400")}>
              {allOnline ? "All Systems Operational" : `${onlineCount}/${totalCount} Environments Online`}
            </p>
            <p className="text-xs text-zinc-500">
              {data.checkedAt ? `Checked at ${new Date(data.checkedAt).toLocaleTimeString()}` : ""}
            </p>
          </div>
            ...
            {data.environments.map((env) => (
              <div
                ...
                className={cn(
                  ...
                  env.status === "online" ? "bg-emerald-500 animate-pulse" : "bg-red-500"
                )}
            ...
            <Card className="bg-gradient-to-br from-primary/10 to-transparent border-primary/20 h-full">
              <CardHeader className="pb-3">
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle className="text-sm font-bold uppercase tracking-widest">API Server</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="space-y-5">
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-white/5">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold font-mono text-emerald-400">{formatUptime(data.server.uptime)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3 p-3 rounded-lg bg-white/5">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-zinc-500 uppercase tracking-widest">Memory</span>
                  </div>
                  <MemoryBar used={data.server.memoryMB.heapUsed} total={data.server.memoryMB.heapTotal} label="Heap" />
                  <MemoryBar used={data.server.memoryMB.rss} total={data.server.memoryMB.heapTotal} label="RSS" />
                </div>

                {/* Runtime Info */}
                <div className="space-y-2">
                  {[
                    { icon: Terminal, label: "Node.js", value: data.server.nodeVersion },
                    { icon: Globe, label: "Platform", value: data.server.platform },
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {env.error && (
                            <p className="text-[10px] text-red-400/70 mt-1 truncate font-mono">{env.error}</p>
                          )}
                        </div>
                        ...
                        {env.latencyMs !== undefined && (
                          <div className="text-right flex-shrink-0">
                            ...
                            <p className={cn(
                              ...
                              env.latencyMs < 500 ? "text-emerald-400" :
                              env.latencyMs < 2000 ? "text-amber-400" : "text-red-400"
                            )}>
                              {env.latencyMs > 1000
